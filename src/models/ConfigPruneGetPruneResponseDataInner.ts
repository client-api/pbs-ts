/* tslint:disable */
/* eslint-disable */
/**
 * Proxmox Backup Server API
 * Generated from apidoc.js. NOT an official Proxmox specification. See https://pbs.proxmox.com/docs/api-viewer/ for the upstream documentation.
 *
 * The version of the OpenAPI document: 9.x
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface ConfigPruneGetPruneResponseDataInner
 */
export interface ConfigPruneGetPruneResponseDataInner {

    /**
     * Comment.
     * @type {string}
     * @memberof ConfigPruneGetPruneResponseDataInner
     */
    comment?: string;

    /**
     * Disable this job.
     * @type {boolean}
     * @memberof ConfigPruneGetPruneResponseDataInner
     */
    disable?: boolean;

    /**
     * Job ID.
     * @type {string}
     * @memberof ConfigPruneGetPruneResponseDataInner
     */
    id: string;

    /**
     * Number of daily backups to keep.
     * @type {number}
     * @memberof ConfigPruneGetPruneResponseDataInner
     */
    keepDaily?: bigint | string | number;

    /**
     * Number of hourly backups to keep.
     * @type {number}
     * @memberof ConfigPruneGetPruneResponseDataInner
     */
    keepHourly?: bigint | string | number;

    /**
     * Number of backups to keep.
     * @type {number}
     * @memberof ConfigPruneGetPruneResponseDataInner
     */
    keepLast?: bigint | string | number;

    /**
     * Number of monthly backups to keep.
     * @type {number}
     * @memberof ConfigPruneGetPruneResponseDataInner
     */
    keepMonthly?: bigint | string | number;

    /**
     * Number of weekly backups to keep.
     * @type {number}
     * @memberof ConfigPruneGetPruneResponseDataInner
     */
    keepWeekly?: bigint | string | number;

    /**
     * Number of yearly backups to keep.
     * @type {number}
     * @memberof ConfigPruneGetPruneResponseDataInner
     */
    keepYearly?: bigint | string | number;

    /**
     * How many levels of namespaces should be operated on (0 == no recursion, empty == automatic full recursion, namespace depths reduce maximum allowed value)
     * @type {number}
     * @memberof ConfigPruneGetPruneResponseDataInner
     */
    maxDepth?: number;

    /**
     * Namespace.
     * @type {string}
     * @memberof ConfigPruneGetPruneResponseDataInner
     */
    ns?: string;

    /**
     * Run prune job at specified schedule.
     * @type {string}
     * @memberof ConfigPruneGetPruneResponseDataInner
     */
    schedule: string;

    /**
     * Datastore name.
     * @type {string}
     * @memberof ConfigPruneGetPruneResponseDataInner
     */
    store: string;

}

/**
 * Check if a given object implements the ConfigPruneGetPruneResponseDataInner interface.
 */
export function instanceOfConfigPruneGetPruneResponseDataInner(value: object): value is ConfigPruneGetPruneResponseDataInner {
    
    
    
    if (!('id' in value) || value['id'] === undefined) return false;
    
    
    
    
    
    
    
    
    
    if (!('schedule' in value) || value['schedule'] === undefined) return false;
    
    if (!('store' in value) || value['store'] === undefined) return false;
    
    return true;
}

export function ConfigPruneGetPruneResponseDataInnerFromJSON(json: any): ConfigPruneGetPruneResponseDataInner {
    return ConfigPruneGetPruneResponseDataInnerFromJSONTyped(json, false);
}

export function ConfigPruneGetPruneResponseDataInnerFromJSONTyped(json: any, ignoreDiscriminator: boolean): ConfigPruneGetPruneResponseDataInner {
    if (json == null) {
        return json;
    }
    return {
        
        
        'comment': json['comment'] == null ? undefined : json['comment'],
        
        'disable': json['disable'] == null ? undefined : json['disable'],
        
        'id': json['id'],
        
        'keepDaily': json['keep-daily'] == null ? undefined : BigInt(json['keep-daily']),
        
        'keepHourly': json['keep-hourly'] == null ? undefined : BigInt(json['keep-hourly']),
        
        'keepLast': json['keep-last'] == null ? undefined : BigInt(json['keep-last']),
        
        'keepMonthly': json['keep-monthly'] == null ? undefined : BigInt(json['keep-monthly']),
        
        'keepWeekly': json['keep-weekly'] == null ? undefined : BigInt(json['keep-weekly']),
        
        'keepYearly': json['keep-yearly'] == null ? undefined : BigInt(json['keep-yearly']),
        
        'maxDepth': json['max-depth'] == null ? undefined : json['max-depth'],
        
        'ns': json['ns'] == null ? undefined : json['ns'],
        
        'schedule': json['schedule'],
        
        'store': json['store'],
        
    };
}

export function ConfigPruneGetPruneResponseDataInnerToJSON(json: any): ConfigPruneGetPruneResponseDataInner {
    return ConfigPruneGetPruneResponseDataInnerToJSONTyped(json, false);
}

export function ConfigPruneGetPruneResponseDataInnerToJSONTyped(value?: ConfigPruneGetPruneResponseDataInner | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        
        'comment': value['comment'],
        
        'disable': value['disable'],
        
        'id': value['id'],
        
        'keep-daily': value['keepDaily'] == null ? undefined : String(value['keepDaily']),
        
        'keep-hourly': value['keepHourly'] == null ? undefined : String(value['keepHourly']),
        
        'keep-last': value['keepLast'] == null ? undefined : String(value['keepLast']),
        
        'keep-monthly': value['keepMonthly'] == null ? undefined : String(value['keepMonthly']),
        
        'keep-weekly': value['keepWeekly'] == null ? undefined : String(value['keepWeekly']),
        
        'keep-yearly': value['keepYearly'] == null ? undefined : String(value['keepYearly']),
        
        'max-depth': value['maxDepth'],
        
        'ns': value['ns'],
        
        'schedule': value['schedule'],
        
        'store': value['store'],
        
    };
}

